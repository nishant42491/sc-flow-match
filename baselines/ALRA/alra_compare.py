...
def compute_scores(original_csv, resultant_csv,dataset_name, dropout_eval_name):
    ...
    resultant_df = pd.read_csv(resultant_csv,skiprows=1, header=None)
    ...
    save_dir = f'baseline_eval/alra/{dataset_name}/{dropout_eval_name}/{dataset_name}_results_log.txt'
...
def compute_cluster_scores(original_csv, resultant_csv,dataset_name, dropout_eval_name):
    ...
    original_df = pd.read_csv(original_csv, header=None)
    resultant_df = pd.read_csv(resultant_csv)
    ...
    save_dir = f'baseline_eval/alra/{dataset_name}/{dropout_eval_name}/{dataset_name}_cluster_results_log.txt'
    Path(save_dir).parent.mkdir(parents=True, exist_ok=True)
    # Write results to a text file
    with open(save_dir, "w") as file:
        file.write(log)
...
if __name__ == "__main__":
    ...
    for dataset in dataset_name:
        for dropout_eval in dropout_eval_name:
            ...
            gen_out = f'baseline_outputs/alra/{dataset}/{dropout_eval}/{dataset}_imputed.csv'
